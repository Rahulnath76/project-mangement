
export const auth = {
    SIGNUP_API: "/auth/signup",
    LOGIN_API: "/auth/login",
    LOGOUT_API: "/auth/logout"
}

export const project = {
    CREATE_PROJECT: "/project/new-project",
    UPDATE_PROJECT: "/project/update-project",
    DELETE_PROJECT: "/project/delete-project/:id",
    GET_PROJECT: "/project/get-project/:id",
    GET_ALL_PROJECTS: "/project/get-all-projects",
}

export const task = {
    CREATE_TASK: "/new-task",
    UPDATE_TASK: "/update-task",
    DELETE_TASK: "/:projectId/delete-task/:taskId",
    GET_ALL_TASKS: "/:projectId/get-all-tasks",
}
